async function handler({
  ...
  try {
    ...
    console.log(`[HANDLER] Start - action: ${action}, userId: ${userIdStr}`);
    ...
    try {
      ...
      `;
      console.log(`[CREDENTIAL CHECK] Rows found: ${userResult?.length || 0}`);
      if (!userResult || userResult.length === 0) {
        console.warn(
          `[CREDENTIAL CHECK] Invalid credentials for userId: ${userIdStr}`
        );
      ...
      return {
        error: "Database error during credential check",
    ...
    if (action === "buyRegularItem") {
      ...
      try {
        ...
        return { error: "Database error fetching user save data" };
      }
      ...
      try {
        ...
        return { error: "Database error updating user save" };
      }
    ...
    if (action === "buyLimitedItem") {
      ...
      try {
        ...
        return { error: "Database error fetching limited item stock" };
      }
      ...
      try {
        ...
        return { error: "Database error updating limited item stock" };
      }
    ...
    if (action === "save") {
      try {
        ...
        const ownedProfileIconsJSON = JSON.stringify(
          gameState.ownedProfileIcons || []
        );
    ...
    if (action === "load") {
      try {
        ...
        // Fetch limited stock data for all items
        let stockRows;
        try {
          stockRows = await sql`
    ...
    if (action === "redeemReferral") {
      // Hardcoded lists for all icons and all themes:
      const allIcons = [
        "tree",
        "seedling",
        "cloudMoon",
      ...
      const codeName =
        typeof code === "string" ? code.toLowerCase().trim() : "";
    ...
    `;
      ...
      if (codeName === "maddox") {
        ...
        let tap_speed_bonus_upgrades =
          Number(save.tap_speed_bonus_upgrades) || 0;
        let house_coins_multiplier =
          (Number(save.house_coins_multiplier) || 0) + 0.5; // If you want to boost this with code
        ...
        if (!ownedThemes.includes("maddoxtheme"))
          ownedThemes.push("maddoxtheme");
        const availableThemes = allThemes.filter(
          (t) =>
            t !== "maddoxtheme" && t !== "seasons" && !ownedThemes.includes(t)
        );
    ...
    `;
      ...
      // --- Andysocial code ---
        ...
        let tap_speed_bonus_upgrades =
          Number(save.tap_speed_bonus_upgrades) || 0;
        let house_coins_multiplier =
          (Number(save.house_coins_multiplier) || 0) + 0.75;
